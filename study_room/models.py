...
class study_room(models.Model):
    ...
    no_of_students = fields.Integer(string="No. of Students", compute='_get_no_of_students', store=True, help="This Field shows the number of related students.")

    @api.one
    @api.constrains('no_of_students')
    def _check_no_of_students(self):
        if self.no_of_students > 2:
            raise Warning('The max is 2, you can not add the third!')
